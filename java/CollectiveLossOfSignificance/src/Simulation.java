...
public class Simulation {
	...
	public Simulation(InputParameterSet input)
	{
		...
		for (int i=0; i<numberOfAgents; i++)
		{
			Agent toInit = new Agent(questValueDistribution, questThresholdDistribution, 
					                 ...
					                 input.getCorrBetweenIdeologyAndQuestValue());
			if (i % 100 == 0)
			{
				/**
	...
	public void step(int timeStep)
	{
		if (this.isCollectiveLossStep(timeStep))
		{
			...
			{
				double curAgentQuestValue = agents.get(i).getQuestForSignficanceValue();
				double incAgentQuestValue = event.getIncToQuestValue(agents.get(i).getThreatSuspectibility());
				double newAgentQuestValue = curAgentQuestValue + incAgentQuestValue;
				agents.get(i).setQuestForSignficanceValue(newAgentQuestValue);
			}
		}
		for (int i=0; i<agents.size(); i++)
		{
